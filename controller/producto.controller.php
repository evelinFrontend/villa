...
Class ProductoController{
    ...
    function readByProduct(){
        ...
        if(!empty($_POST)){
            $request = $_POST;
            $dataType = $this->masterModel->sqlSelect("SELECT p.pro_codigo,p.pro_nombre,c.cat_nombre,pv.pr_nombre,p.id_producto,c.id_categoria,pv.id_proveedor,p.pro_imagen FROM producto p INNER JOIN categorias c ON p.id_categoria = c.id_categoria INNER JOIN proveedores pv ON p.id_proveedor = pv.id_proveedor WHERE  ".$request['columnDBSearch']." = ? AND pro_estado = ?  ",array($request["value"],1));
            if(!empty($dataType)){
                $status = "success";
